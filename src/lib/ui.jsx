...
export function StyledName({ user }) {
  ...
  const nameColor = user?.nameColor || user?.authorNameColor || "";
  const nameStyle = user?.nameStyle || user?.authorNameStyle || "";
  if (nameColor === "gold" || nameStyle === "gold") return <span className="name-gold">{name}</span>;
  if (nameStyle === "grad") {
    const baseColor = nameColor && nameColor !== "gold" ? nameColor : "#f8fafc";
    return <span className="name-grad-anim" style={{ "--name-grad-base": baseColor }}>{name}</span>;
  }
  if (nameStyle === "glow") {
    const baseColor = nameColor && nameColor !== "gold" ? nameColor : "#ec4899";
    return <span className="name-glow" style={{ "--name-glow-base": baseColor, color: baseColor }}>{name}</span>;
  }
  if (nameColor) return <span style={{ color: nameColor }}>{name}</span>;
  return <>{name}</>;
}
...
export function RoleBadges({ user }) {
  const showInfo = (role, event) => {
    ...
    const symbol = isAdmin
      ? `<span class="admin-badge role-badge role-badge-static" aria-hidden="true"><svg viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M12 2l2.39 5.46L20 8.27l-4.19 3.94L17.03 18 12 15.27 6.97 18l1.22-5.79L4 8.27l5.61-.81L12 2z"/></svg></span>`
    ...
    wrap.innerHTML = `
      <div class="role-info-card" style="background:#121212;border:1px solid var(--border);border-radius:18px;padding:24px 22px 18px;width:min(92vw,360px);box-shadow:0 24px 60px -10px rgba(0,0,0,.7);text-align:center;">
        <div class="role-info-symbol ${isAdmin ? "is-admin" : "is-mod"}">${symbol}</div>
